...
def generateCosets(cipher, length):
    cosetArray = []

    for i in range(0,length):
        coset = cipher[i::length]
        i = i+1
        cosetArray.append(coset)

    cosetIC = generateIC(cosetArray)
    return cosetIC

def crack(cipher):
    cosetICArray = []
    for i in range(1,11):
        cosetICArray.append(generateCosets(cipher, i))

    print("IC for key lengths from 1 to 10 is: " + str(cosetICArray))
    print("Max IC is: " + str(np.max(cosetICArray)))
    print("Most probable key length is: " + str(np.argmax(cosetICArray) + 1))
